...
IOHANDLE io_open_impl(const char *filename, int flags)
{
	...
	if((flags & IOFLAG_READ) != 0)
	{
		...
		if(handle == INVALID_HANDLE_VALUE)
			return 0x0;
		WideCharToMultiByte(CP_UTF8, 0, finddata.cFileName, -1, buffer, sizeof(buffer), nullptr, nullptr);
...
#elif defined(CONF_FAMILY_WINDOWS)
	return CreateThread(nullptr, 0, thread_run, data, 0, nullptr);
#else
#error not implemented
#endif
}
...
void sphore_signal(SEMAPHORE *sem) { ReleaseSemaphore((HANDLE) *sem, 1, nullptr); }
...
static int priv_net_create_socket(int domain, int type, struct sockaddr *addr, int sockaddrlen, int use_random_port)
{
	...
	if(sock < 0)
	{
		...
		if(FormatMessageW(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, 0, error, 0, wBuffer, sizeof(wBuffer) / sizeof(WCHAR), 0) == 0)
			wBuffer[0] = 0;
		WideCharToMultiByte(CP_UTF8, 0, wBuffer, -1, buf, sizeof(buf), nullptr, nullptr);
...
void fs_listdir(const char *dir, FS_LISTDIR_CALLBACK cb, int type, void *user)
{
	...
	do
	{
		WideCharToMultiByte(CP_UTF8, 0, finddata.cFileName, -1, buffer2, sizeof(buffer2), nullptr, nullptr);
...
int fs_storage_path(const char *appname, char *path, int max)
{
	...
	if(!home)
		return -1;
	WideCharToMultiByte(CP_UTF8, 0, home, -1, buffer, sizeof(buffer), nullptr, nullptr);
...
char *fs_getcwd(char *buffer, int buffer_size)
{
	...
	WideCharToMultiByte(CP_UTF8, 0, wBuffer, -1, buffer, buffer_size, nullptr, nullptr);
	return buffer;
#else
