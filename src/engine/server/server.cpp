...
{
	if(ClientID < 0 || ClientID >= MAX_PLAYERS || m_aClients[ClientID].m_State < CClient::STATE_READY || !pName)
		return;
...
{
	dbg_assert(ClientID >= 0 && ClientID < MAX_PLAYERS, "client_id is not valid");
