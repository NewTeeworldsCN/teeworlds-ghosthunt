...
void CCharacterCore::Tick(bool UseInput, bool PhysicLayer)
{
	m_TriggeredEvents = 0;

	// get ground state
	const bool Grounded =
		m_pCollision->CheckPoint(m_Pos.x + PHYS_SIZE / 2, m_Pos.y + PHYS_SIZE / 2 + 5, PhysicLayer) || m_pCollision->CheckPoint(m_Pos.x - PHYS_SIZE / 2, m_Pos.y + PHYS_SIZE / 2 + 5, PhysicLayer);
	...
	else if(m_HookState == HOOK_FLYING)
	{
		...
		int Hit = m_pCollision->IntersectLine(m_HookPos, NewPos, &NewPos, 0, PhysicLayer);
		if(Hit)
		{
...
void CCharacterCore::Move(bool PhysicLayer)
{
	if(!m_pWorld)
		return;
	...
	vec2 NewPos = m_Pos;
	m_pCollision->MoveBox(&NewPos, &m_Vel, vec2(PHYS_SIZE, PHYS_SIZE), 0, &m_Death, PhysicLayer);
